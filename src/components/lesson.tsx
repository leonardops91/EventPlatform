...
export default function Lesson(props: iProps) {
  ...
  const formatedAvailableAt = format(
    props.availableAt, "iiii ' • ' d ' de ' MMMM ' • ' H'h'mm", {locale: ptBR})
  return (
    <Link to={`/event/lesson/${props.slug}`} className="group">
      <span className="text-gray-400 mb-2 block">{formatedAvailableAt.toString()}</span>
      <div className="border border-gray-600 rounded-[4px] p-4 group-hover:border-purple">
        <header className="flex justify-between">
          ...
          <span className="border border-purple rounded-md text-purple py-0.5 px-2 text-sm">
